...
In version ___, Sanic announced that v23.3 would deprecate allowing routes to be registered with duplicate names. If you see the following error, it is because of that change:

```
...
```

Hopefully you were able to make the changes to your application in the meantime and do no come across this issue. However, one instance that may require further changes is IF you have multiple host names attached to a route. For example:
...
In this example, the name of the routes will be as follows:

- `test_bp_host.handler_example_com`
- `test_bp_host.handler_sub_example_com`

As you can see, the names have been appended with the host name where all `.` have been converted to `_`.
