...
- 一个可在应用程序中的任何地方使用的变量，用于暂停执行，直到事件被触发。
- 触发事件，并运行信号处理程序
...
如果您的路由参数使用了非 ascii 码的字符，Sanic 将不再为您进行解码，您需要显式传参来告诉路由它应该为您解码。 You will need to specifically tell the route definition that it should do so.
...
The `Sanic` and `Blueprint` classes share a common base. Previously they duplicated a lot of functionality, that lead to slightly different implementations between them. 既然他们都继承自同一个基类，开发者和插件应该有一个统一的 API 来进行工作。
...
We have split the documentation into two. The docstrings inside the codebase will still continue to build sphinx docs to ReadTheDocs. However, it will be limited to API documentation. 新的前端页面将使用 “Sanic 用户指南”。
