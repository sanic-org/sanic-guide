...
21.3 版本中包含了[处理流式传输的重大变更](https://sanicframework.org/zh/guide/release-notes/v21.3.html#%E6%9B%B4%E6%96%B0%E5%86%85%E5%AE%B9-what-to-know)。新的使用方式（见下文）将代替原先的方式。为了方便起见，我们加入了一个新的 `response.eof()` 方法。一旦所有的数据被推送到客户端，就该调用该方法。
...
目前的版本为 `conn_info` 对象增加了一个新的 `clien_ip` 属性。对于 IPv4 ，您不会注意到有什么不同。然而，对于 IPv6 的应用，新的属性将提供一个“无包装”的地址版本。请看下面的例子：
